    ...
    <div class="container-fluid px-0">
        ...
        <div class="d-flex flex-wrap justify-content-between align-items-center mb-3 gap-3 shadow-sm p-3 rounded-4 zedan-page-header">
            <div class="d-flex align-items-center gap-2">
                ...
                <div>
                    <h3 class="mb-0">{{ __('assets.title') }}</h3>
                </div>
            </div>
            ...
            <form method="GET" class="d-flex flex-wrap gap-2 flex-grow-1 justify-content-end" role="search">
                <div class="input-group" style="max-width: 320px;">
                    ...
                    <input id="assetSearch" type="search" name="search" value="{{ request('search') }}" class="form-control" placeholder="{{ __('assets.search') }}" maxlength="100" autocomplete="off">
                </div>
                ...
                <button type="button" class="btn btn-primary text-nowrap" data-bs-toggle="modal" data-bs-target="#createAssetModal">
                    <i class="bi bi-plus-lg me-1"></i>{{ __('assets.add_asset') }}
                </button>
            </form>
        </div>
                    ...
                    <table class="table zedan-responsive-table mb-0 align-middle">
                        <thead>
                            <tr>
                                ...
                                <th>{{ __('assets.purchase_date') }}</th>
                                <th>{{ __('assets.purchase_cost') }}</th>
                                <th>{{ __('assets.book_value') }}</th>
                        ...
                        <tbody>
                            @forelse ($assets as $asset)
                                <tr>
                                    ...
                                    <td data-label="{{ __('assets.purchase_cost') }}">
                                        <span data-compact-money data-value="{{ $asset->purchase_cost }}">{{ number_format($asset->purchase_cost, 2) }}</span>
                                    </td>
                                    <td data-label="{{ __('assets.book_value') }}">
                                        <span class="fw-semibold">
                                            <span data-compact-money data-value="{{ $asset->bookValue() }}">{{ number_format($asset->bookValue(), 2) }}</span>
                                        </span>
                                    </td>
                                    <td data-label="{{ __('assets.recorded_by') }}">{{ $asset->creator?->name ?: '—' }}</td>
                                    <td data-label="{{ __('assets.actions') }}" class="text-end">
                                        <div class="d-inline-flex gap-1">
                                            <button type="button" class="btn btn-sm btn-outline-primary" data-bs-toggle="modal" data-bs-target="#editAssetModal{{ $asset->id }}" title="{{ __('assets.edit') }}">
                                                <i class="bi bi-pencil"></i>
                                            </button>
                                            <form action="{{ route('assets.destroy', $asset) }}" method="POST" class="d-inline" onsubmit="return confirm('{{ __('assets.confirm_delete') }}')">
                                                @csrf
                                                @method('DELETE')
                                                <button class="btn btn-sm btn-outline-danger" title="{{ __('assets.delete') }}">
